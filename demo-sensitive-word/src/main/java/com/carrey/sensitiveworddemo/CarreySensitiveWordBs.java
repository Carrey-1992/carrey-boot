...
/**
 * @author Conway
 * @className CarreySensitiveWordBs
 * @description
 * @date 2020/7/23 6:35 下午
 */
public class CarreySensitiveWordBs {
    /**
     * 私有化构造器
     * @since 0.0.1
     */
    private CarreySensitiveWordBs(){}
    ...
    private static IWordMap initWordMap() {
        if(sensitiveWordMap == null) {
            synchronized (IWordMap.class) {
                if(sensitiveWordMap == null) {
                    // 加载配置信息
                    IWordData wordData = new CarreySensitiveWordData();
     ...
     * @since 0.0.1
     */
    public static CarreySensitiveWordBs newInstance() {
        initWordMap();

        CarreySensitiveWordBs bs = new CarreySensitiveWordBs();
